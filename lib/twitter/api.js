...
export async function fetchTweetsHtml(ids) {
  ...
  console.log('LOG', `${SYNDICATION_URL}/tweets.json?ids=${ids}`, res.status)
...
export async function fetchPartnerColor(name) {
  const url = `https://mockend.com/tungduonghgg123/mockend-server/partner?name_eq=${name}`
  ...
  throw new Error(
    `Fetch for the embedded tweets of "${name}" failed with code: ${res.status}`
  )
}
